...
def run_trial(
    ...
    # Hyperparameters for the new MPNN architecture
    # message_units is restricted to 32, 64, 128 so node_dim remains divisible by attention heads
    message_units = trial.suggest_categorical("message_units", [32, 64, 128])
    message_steps = trial.suggest_int("message_steps", 2, 6)
    ...
    fusion_dropout = trial.suggest_float("fusion_dropout", 0.1, 0.5, step=0.1)
...
if __name__ == "__main__":
    ...
    model = MPNN(
        message_units=best["message_units"],
        message_steps=best["message_steps"],
