...
##### ATTENTION: JUST NEED TO CHANGE THE PATH HERE!!!!!!!!!!!#######
...
# remove identical columns with just different names (this is determined by manual checking):
# remove TPSA.1:
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.15, stratify=y, random_state=67)
X_train_model, X_val, y_train_model, y_val = train_test_split(X_train, y_train, test_size=10/85, stratify=y_train, random_state=67)

hp_para = [5, 10, 15, 50, 85, 100]

# set up empty lists to store results for different numbers of predictors:
acc_res_in_val_Gaussian = []
f1_results_in_val_Gaussian = []
roc_auc_in_val_Gaussian = []


for i in hp_para:
    print(f"Selection number be the union of both {i} predictors")

    # ATTENTION:
    # ATTENTION:
    # ATTENTION:
    # use any number to select top predictors, I am lazy to change the name of the variables,
    # so all the numbers in the variables in the code is 6

    # use random forest's importance ranking to select top 6 predictors, this represents NON-LINEAR:
    rf = RandomForestClassifier(n_estimators=400, random_state=67)
    ...
    top_6_features_rf = feature_importance_df.head(i)["feature"].tolist()
    ...
    lasso = LassoCV(cv=5, random_state=67, max_iter=10000)
    ...
    top_6_features_lasso = non_zero_coef.sort_values(by="abs_coef", ascending=False).head(i)["feature"].tolist()
    ...
    removed_predictors = ["NumHDonors", "NumHeteroatoms", "NOCount"]
    ...
    print(f1_score(y_val, y_val_pred_g))
    ...
    # lastly selected numbe rof predictors:
    num_predictors = len(best_features)
    print(f"Number of predictors selected: {num_predictors}")
    
    # the number of the hp_para in this iteration:
    print(f"Current hp_para (number of predictors selected by each method): {i}")

    # store the acc, f1, and roc_auc for this number of predictors:
    acc_res_in_val_Gaussian.append(accuracy_score(y_val, y_val_pred_g))
    f1_results_in_val_Gaussian.append(f1_score(y_val, y_val_pred_g))
    roc_auc_in_val_Gaussian.append(roc_auc_g)


# plot the acc, f1, and roc_auc lines in a same figure with respect to the num_predictors:
plt.figure(figsize=(8, 6))
# add legend for the lines:
plt.plot(hp_para, acc_res_in_val_Gaussian, marker="o")
plt.plot(hp_para, f1_results_in_val_Gaussian, marker="o", color="orange")
plt.plot(hp_para, roc_auc_in_val_Gaussian, marker="o", color="green")
plt.xlabel("Number of Predictors Selected")
plt.ylabel("Validation Performance")
plt.title("Validation Performance vs Number of Predictors in each Method (GaussianNB)")
plt.legend(["Accuracy", "F1 Score", "ROC AUC"])
plt.tight_layout()
plt.show()

# From the plot comparison, we will choose hp_para = 10 as the optimal number.

# the following code is using the optimal number of predictors (10) to determine the weights of the ensemble model:
...
top_6_features_rf = feature_importance_df.head(10)["feature"].tolist()
print("Top 6 predictors:", top_6_features_rf)
...
non_zero_coef = lasso_coefs[lasso_coefs["coef"] != 0].copy()
non_zero_coef["abs_coef"] = non_zero_coef["coef"].abs()
top_6_features_lasso = non_zero_coef.sort_values(by="abs_coef", ascending=False).head(10)["feature"].tolist()
print("Top 6 predictors from LassoCV:", top_6_features_lasso)
...
print("optimal validation scores for GNB:")
...
print(f1_score(y_val, y_val_pred_b))
...
# before ensemble, we will test the weights for the two models on validation set:
g_weights = [0, 0.1, 0.15, 0.2, 0.3, 0.5, 0.7, 0.8, 0.85, 0.9, 1]
b_weights = [1, 0.9, 0.85, 0.8, 0.7, 0.5, 0.3, 0.2, 0.15, 0.1, 0]
wt_results = []
for g_w, b_w in zip(g_weights, b_weights):
    prob_ensemble_in_val = g_w * y_val_prob_g + b_w * y_val_prob_b
    y_pred_ensemble_in_val = (prob_ensemble_in_val >= 0.5).astype(int)
    acc_ensemble_in_val = accuracy_score(y_val, y_pred_ensemble_in_val)
    f1_ensemble_in_val = f1_score(y_val, y_pred_ensemble_in_val)
    roc_auc_ensemble_in_val = roc_auc_score(y_val, prob_ensemble_in_val)
    print(f"Ensemble Weights - GaussianNB: {g_w}, BernoulliNB: {b_w}")
    print(f"Validation Accuracy (Ensemble): {acc_ensemble_in_val:.4f}")
    print(f"Validation F1 Score (Ensemble): {f1_ensemble_in_val:.4f}")
    print(f"Validation AUROC (Ensemble): {roc_auc_ensemble_in_val:.4f}\n")

    # append the results for this weight combination:
    wt_results.append({
        "GaussianNB_weight": g_w,
        "BernoulliNB_weight": b_w,
        "Validation_Accuracy": acc_ensemble_in_val,
        "Validation_F1": f1_ensemble_in_val,
        "Validation_AUROC": roc_auc_ensemble_in_val
    })

# plot the validation performance of different weight combinations:
plt.figure(figsize=(10, 6))
plt.plot(g_weights, [r["Validation_Accuracy"] for r in wt_results], marker="o", label="Validation Accuracy")
plt.plot(g_weights, [r["Validation_F1"] for r in wt_results], marker="o", label="Validation F1 Score")
plt.plot(g_weights, [r["Validation_AUROC"] for r in wt_results], marker="o", label="Validation AUROC")
plt.xlabel("GaussianNB Weight (BernoulliNB weight = 1 - GaussianNB weight)")
plt.ylabel("Validation Performance")
plt.title("Validation Performance vs Ensemble Weights")
plt.legend()
plt.xticks(g_weights)
plt.tight_layout()
plt.show()

# lastly these are the results in the test set:

# We ensemble these two models, as though Bernoulli NB performs better in most of the cases, while GNB is better at recall.
# we will trust Bernoulli NB more (0.85), but still give some weight to Gaussian NB (0.15):
# predictions
...
# with soft ensemble for AUROC:
prob_ensemble = 0.15 * prob_g + 0.85 * prob_b

# by our ensemble prediction rule
y_test_pred_ensemble = (prob_ensemble >= 0.5).astype(int)
...
print(f1_score(y_test, y_test_pred_ensemble))

print("\nAUROC Test Score (Ensemble NB Version):")
print(roc_auc_score(y_test, prob_ensemble))

# the final plot of the ROC curve for the ensemble model:
fpr_ensemble, tpr_ensemble, _ = roc_curve(y_test, prob_ensemble)
roc_auc_ensemble = auc(fpr_ensemble, tpr_ensemble)
plt.figure(figsize=(4, 4))
plt.plot(fpr_ensemble, tpr_ensemble, label=f'AUC = {roc_auc_ensemble:.3f}')
plt.plot([0, 1], [0, 1], "k--")
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
...
plt.title("Confusion Matrix for Ensemble NB Model")
plt.tight_layout()
plt.show()
